...
def record_consent(
    ...
    if not session.before_image_key:
        raise HTTPException(status_code=400, detail="Before image is required before recording consent")
    ...
    if payload.consent_tier.value == ConsentTier.decline.value:
        ...
        if practice.auto_publish:
            ...
            session.status = next_status_after_consent(practice)
    # else: after image not yet uploaded — status stays as-is until the patient delivers it
