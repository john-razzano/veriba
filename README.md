...
6. Visit `http://localhost/veriba-admin/` for the hidden internal admin portal.
7. Visit `http://localhost:8000/docs` for the API docs.
...
- `./scripts/internal_admin_smoke_test.sh` verifies the hidden internal admin shell and internal API flow.
...
- The Docker `fastapi` container auto-provisions the dev internal admin account on startup.
- `./scripts/seed_internal_admin.sh` re-provisions the dev internal admin account and resets it to the documented credentials.
- [INTERNAL-ADMIN.md](INTERNAL-ADMIN.md) documents the hidden route, credentials, and smoke test.
...
- `frontend` contains the public gallery site, medspa admin studio, and hidden internal admin portal served by Caddy.
